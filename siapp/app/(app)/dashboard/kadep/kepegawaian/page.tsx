...
import { dosenData, currentUser } from '@/lib/mock-data-kadep'
...
export default function KepegawaianPage() {
  const [dosens, setDosens] = useState<Dosen[]>(dosenData as Dosen[])
  ...
  return (
    ...
    >
      {/* Summary cards — derived from live state */}
      ...
      {/* Table with CRUD — shares state with summary above */}
      <DosenTable dosens={dosens} onChange={setDosens} />

      {/* Profile grid — same state */}
        ...
        <div className="grid grid-cols-2 gap-3">
          {dosens.map(d => (
            <div key={d.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors">
              ...
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${jabatanColor[d.jabatan]}`}>
                {d.jabatan.split(' ')[0]}
              </span>
            </div>
          ))}
