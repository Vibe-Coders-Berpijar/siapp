...
interface Props { userName: string; jabatan: string }

export default function AkreditasiClient({ userName, jabatan }: Props) {
  const [details, setDetails] = useState<CriterionDetail[]>(criteriaDetails)

  const criteria = details.map(d => ({
    kode: d.kode, label: d.label, score: computeCriterionScore(d.metrics),
  }))
  ...
  return (
    <DashboardLayout title="Akreditasi BAN-PT" subtitle="Pemantauan kesiapan akreditasi 9 kriteria LAMSPAK · skor dihitung dari indikator nyata" userName={userName} jabatan={jabatan}>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Bukti Hibah — K7 Penelitian &amp; K9 Keluaran Tridarma</p>
            <p className="text-[10px] text-slate-400 mt-0.5">Data dari Modul Riset · hanya tampilan</p>
          </div>
          <button className="text-xs font-medium px-3 py-1.5 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 transition-colors">Kelola di Modul Riset →</button>
        </div>
