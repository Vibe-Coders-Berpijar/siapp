...
export default function PersuratanKadepClient({ initialSurat, userName, jabatan }: Props) {
  ...
  function openEdit(s: Surat) {
    ...
    setEditId(s.id); setEditOriginalStatus(s.status); setModal('edit')
  }
  ...
  function handleSave() {
    ...
    if (modal === 'add') {
      ...
      const maxSeq = Math.max(...surats.map(s => { const m = s.nomor.match(/B\/(\d+)\//) ; return m ? parseInt(m[1]) : 0 }), 0)
  ...
  function set<K extends keyof typeof form>(k: K, v: typeof form[K]) { setForm(prev => ({ ...prev, [k]: v })) }

  const filtered = surats.filter(s => activeTab === 'Semua' || s.status.toLowerCase() === activeTab.toLowerCase())
  const counts = {
    total: surats.length,
  ...
  return (
    <DashboardLayout title="Persuratan" subtitle="Kelola dan tandatangani surat keluar departemen" userName={userName} jabatan={jabatan}>
      {node}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        ...
        {filtered.length === 0 && <p className="text-sm text-slate-400 text-center py-10">Tidak ada surat ditemukan.</p>}
            ...
            <div key={surat.id} className="flex items-start justify-between gap-4 p-4 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {surat.status === 'menunggu' && (
                  <button onClick={() => handleSign(surat.id, surat.perihal)} className="text-xs font-medium px-3 py-1 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors">Tanda Tangani</button>
                )}
          ...
          <Field label="Status">
            {modal === 'add' ? (
              ...
              <select className={selectCls} value={form.status} onChange={e => set('status', e.target.value as SuratStatus)}>
                ...
                {(editOriginalStatus === 'menunggu' || editOriginalStatus === 'ditandatangani') && <option value="ditandatangani">Ditandatangani</option>}
                {(editOriginalStatus === 'menunggu' || editOriginalStatus === 'ditandatangani' || editOriginalStatus === 'diarsip') && <option value="diarsip">Diarsip</option>}
                {(editOriginalStatus === 'menunggu' || editOriginalStatus === 'ditolak') && <option value="ditolak">Ditolak</option>}
              </select>
            )}
          </Field>
        </Modal>
